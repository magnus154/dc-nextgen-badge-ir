...
The Flipper's IR receiver only listens for one blink speed, built into its hardware. This badge doesn't blink the way that receiver expects. Every recording we made came back as junk. Every guess we sent back got ignored. Points stayed at 21 no matter what.
...
We watched the Flipper's live feed for 25 to 40 second stretches, doing several touches in a row. We also held the badge's little contact button down the whole time, in case it only sends anything while that's pressed. We never confirmed that it does, but held it throughout to be safe.
...
That rules out bad aim. If the receiver could hear this badge at all, holding them together for 25 seconds would catch something steady, not silence one time and three random blips the next. It just can't hear this badge, though at this point we didn't yet know why.
...
At first we read that as the badge checking something, a format or a checksum, before counting a touch. We're leaving that guess here on purpose, because it doesn't hold up. Every signal we fired in Try 3 was carrier-modulated, the standard remote kind. We later learned from Relic that the badge uses no carrier at all (see below), so none of these were even the right kind of blinking for the badge's sensor to read. They'd have landed as flicker, not the clean on/off pulses the badge expects. That means the badge could have ignored all 48 before doing any checking of its own.

So Try 3 proves less than we thought. All it really shows is that carrier-modulated remote signals don't score. Whether the badge checks a format, or a checksum, or anything else past the raw signal, is still open. We haven't sent it a single well-formed baseband message, let alone watched one get rejected. We need to see a real message first.
...
- **It does not use a carrier.** Our three noise captures were consistent with this, though on their own they couldn't tell no carrier apart from a carrier at a speed the Flipper can't read. Relic's word settles it: not a carrier far from the Flipper's speed, no carrier at all. The badge just switches its IR light on and off directly. No fast blinking underneath. This is exactly why the Flipper's receiver never stood a chance, it expects a carrier to be there.
...
The Flipper's IR side is really two separate pieces. Sending is simple, just a plain LED that can be switched at nearly any speed, which is how the speed-sweep in Try 3 worked. Receiving is the hard part: a sealed chip that only picks up a signal blinking at its one built-in carrier speed, with no way to change it. It expects that carrier to be there. This badge has no carrier at all, so the chip has nothing to lock onto. Turning blinks into data happens inside that chip, before the Flipper's own software even gets involved. No carrier in, garbage out.
...
There's a coin battery, and what looks like a small power circuit: a small coil marked `L1`, a bigger one marked `KLJ-1230`, and a small transistor marked `Q2`. This is almost certainly what turns the coin cell's 3 volts into whatever the display needs to redraw. The screen looks like e-paper, going by how it behaves: it holds its picture with no power and is slow to change. If that's right, it only needs power to change what it shows. Worth remembering when testing, a slow-changing screen can look like nothing happened when something actually did.
...
A clean firmware read should hand us the exact signal format directly: the timing, the framing, and whether there's a checksum and how it works, without ever needing to guess from the outside.
...
Things that could still complicate this: only having one badge, which makes seeing a real two-badge exchange harder even with the firmware in hand. And if there is a checksum, one that changes with every touch using something the firmware alone might not fully explain, like a shared secret or a counter that ticks up outside of what a single read shows.
